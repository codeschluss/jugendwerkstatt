...
const MediaLibrary: React.FC = () => {
  ...
  const scrollPrevious = (event: any) => {
    let divElementForScroll =
      event.currentTarget.parentNode.nextSibling.children[1];
    let scrollXposition = divElementForScroll.scrollLeft;

    divElementForScroll.scrollTo({
      top: 0,
      left: scrollXposition - 246,
      behavior: "smooth",
    });
  };
  ...
  const scrollNext = (event: any) => {
    let divElementForScroll =
      event.currentTarget.parentNode.previousSibling.children[1];
    let scrollXposition = divElementForScroll.scrollLeft;
  ...
  function showOrHideArrows() {
    if (window.innerWidth >= 1024) {
      ...
      for (var i = 0, len = elements.length; i < len; i++) {
        ...
        var isOverFlowing =
          singleElement.scrollWidth > singleElement.clientWidth;

        let previousArrowElement: any =
          singleElement.parentNode?.previousSibling;
  ...
  const fetchedData: [LinkCategoryEntity] = result.data?.getLinkCategories
      ...
      {fetchedData?.map((singleTopic) => {
        return (
          <div className="relative">
              ...
              >
                ...
                >
                  ...
                  <g filter="url(#grey-shadow)">
                    <path
                      d="M23.78 5.1598L21.4066 2.7998L8.21997 15.9998L21.42 29.1998L23.78 26.8398L12.94 15.9998L23.78 5.1598Z"
                      fill="white"
                    />
                  </g>
                </svg>
            ...
            <Slider
              key={`${singleTopic?.id}`}
              topicTitle={`${singleTopic?.name}`}
            >
              {singleTopic?.links?.map((singleVideo) => {
                return (
                  <>
                    <SlideCard
                      key={`${singleVideo?.id}`}
            ...
            <div className="absolute h-[9rem] z-10 right-0 top-0">
              ...
              >
                ...
                >
                  ...
                  <g filter="url(#grey-shadow)">
                    <path
                      d="M8.15356 26.9732L10.5136 29.3332L23.8469 15.9998L10.5136 2.6665L8.15356 5.0265L19.1269 15.9998L8.15356 26.9732Z"
                      fill="white"
                    />
                  </g>
                </svg>
